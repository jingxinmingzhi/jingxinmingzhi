...
s.verify = False
s.mount('http://', HTTPAdapter(pool_connections=1000, pool_maxsize=1000))
s.mount('https://', HTTPAdapter(pool_connections=1000, pool_maxsize=1000))
import copy
...
class HTMLtoPDF(object):
    ...
    def run(self):
        ...
        for i in range(self.threads_count):
            ...
            threads.append(t)   #将这个子线程添加到线程列表中

        for t in threads:       #遍历线程列表
            t.setDaemon(True)   #把生产线程列为守护线程，否则主线程结束之后不会销毁该线程，程序不会停止
            t.start()           #启动子线程
        print('a')
        ...
        pdfkit.from_file(self.htmls_saved, output, options=options,configuration=config)
        print(output)
        print('done')
        ...
        while True:
            ...
            try:
                if not self.links_queue.empty():
                    ...
                    filename = u'{}_{}_{}.html'.format(num, re.sub(r'[^\w\s()_-]', '', title),
                                                       int(time.time()))  # os.path.abspath('en/abc.html')合成路径 不能是 /en。。
